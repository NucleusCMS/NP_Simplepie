...
var $template_data = array();
            ...
            switch ($func_name)
            {
                case 'image_url':
                    ...
                    {    $title = $this->feed->get_image_title();
                        ...
                        $value = sprintf('<img src="%s" alt="%s" %s />',$value,$title,$add_opt);
                    }
                    break;
                default:
    ...
    function item_parse_callback($match)
    {
        ...
        $func = 'get_' . $func_name;
        if (preg_match('#^add_to_#',$func_name)) $func = $func_name;
            ...
            switch ($func_name)
            {
                ...
                case 'permalink':
                    ...
                    if (preg_match('@http://www\.bidders\.co\.jp/item/(\d+)@',$value,$matches))
                         $value = sprintf('http://www.bidders.co.jp/pitem/%s/aff/%s/%s/IT', $matches[1], BIDDERS_AFID, BIDDERS_LINKID);
                    break;
                case 'author':
                    $value = '';
                    if ($author = $this->feed_item->get_author()) $value = $author->get_name();
                    break;
                case 'category':
                    $value = '';
                    if ($category = $this->feed_item->{$func}())  $value = $category->get_label();
                    break;
                case 'enclosure':
    ...
    {
        if (!($feedURL = $param[PARAM_URL])) return '';
        
        if (isset($param[PARAM_EXTCLASS]) && $param[PARAM_EXTCLASS] !== '')
            $extclass = $param[PARAM_EXTCLASS];
        else
            $extclass = '';
        
        if (isset($param[PARAM_TEMPLATE]) && $param[PARAM_TEMPLATE] !== '')
            $template = $param[PARAM_TEMPLATE];
        else
            $template = $this->getOption(default_template);
        
        if (isset($param[PARAM_AMOUNT]) && 0<intval($param[PARAM_AMOUNT]))
            $amount = intval($param[PARAM_AMOUNT]);
        else
            $amount = intval($this->getOption(default_amount));
        
        if (isset($param[PARAM_STEP]) && 0 < intval($param[PARAM_STEP]))
            $step = intval($param[PARAM_STEP]);
        else
            $step = 0;
        ...
        if (is_file(dirname(__FILE__) . '/simplepie/simplepie.inc'))
        ...
        {    
            if ($extclass == '') $extclass = $this->getOption(default_itemclass);
        ...
        if ($this->getOption(enable_cache) === 'yes')    // Cache Enable??
        {
            ...
            if(isset($param[PARAM_CACHE_TIME]) && intval($param[PARAM_CACHE_TIME]) > 0)
                $cache_time = intval($param[PARAM_CACHE_TIME]);
            else
                $cache_time = intval($this->getOption(cache_time));
            
            $this->feed->set_cache_duration($cache_time);
            $this->feed->enable_cache(true);
        }
        else $this->feed->enable_cache(false);
        ...
        $pattern = '#<%([a-z_]+|([^\(]+)\(([^\)]+)\))%>#msiU';
        $output = preg_replace_callback($pattern,array(&$this,'site_parse_callback'),$this->template_data['ITEM_HEADER']);
        $i = 0;
        ...
        { 
            if (++$i <= $amount)
            {
                ...
                $item_text = preg_replace_callback($pattern,array(&$this,'item_parse_callback'),$this->template_data['item']);
                if ($step) $mod = ($i%$step)+1;
                else       $mod = $i;
        ...
        $footer= preg_replace_callback($pattern,array(&$this,'site_parse_callback'),$this->template_data['ITEM_FOOTER']);
        $output .= $footer;
        return $output;
    }
